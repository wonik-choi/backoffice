...
const scheduleSettingCardButtonVariants = cva(
 ...
 */
export const ScheduleSettingCardButton = ({ onClick, disabled }: { onClick: () => void; disabled: boolean }) => {
  const scheduleSettingCard = useCallback(() => {
    if (disabled) return;
    onClick();
  }, []);
  ...
  return (
    <div
      role="button"
      tabIndex={0}
      className={cn(scheduleSettingCardButtonVariants({ disabled }))}
      onClick={scheduleSettingCard}
      onKeyDown={(e) => {
        if (e.key === 'Enter') {
          scheduleSettingCard();
        }
 ...
 */
export const EditScheduleSettingCard = ({
  name,
  dayOfWeek,
  startAt,
...
type SSC = React.FC<ScheduleSettingCardProps> & {
  Button: typeof ScheduleSettingCardButton;
  Card: typeof EditScheduleSettingCard;
};
...
ScheduleSettingCard.Button = ScheduleSettingCardButton;
ScheduleSettingCard.Card = EditScheduleSettingCard;
