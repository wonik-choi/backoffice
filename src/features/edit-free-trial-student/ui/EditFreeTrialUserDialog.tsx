...
import { patchFreeTrialSchema, PatchFreeTrialSchema } from '@/features/edit-free-trial-student/config/schema';
import { SchemaSectionLayout } from '@/features/register-free-trial/ui/SchameSectionLayout';
import { gradeOptions, inflowOptions, semesterOptions } from '@/features/register-free-trial/config/const';
import ScheduleSettingCard from './ScheduleSettingCardButton';
...
import { initialEditDialogFormConverter } from '@/features/edit-free-trial-student/models/converters/initialEditDialogFormConverter';
import { editFreeTrialUserService } from '@/features/edit-free-trial-student/services/context/EditFreetrialUserService';
import { usePatchFreeTrialUser } from '@/features/edit-free-trial-student/services/query/usePatchFreeTrialUser';
import { patchFreeTrialUserRequestDtoConverter } from '@/features/edit-free-trial-student/models/converters/patchFreeTrialUserRequestDtoConverter';
// views (말이안됨.. 수정 필요) dto 만 보내서 해결하는쪽으로 갈 필요 있음
import { ExpandedFreeTrialUsersTableRowData } from '@/views/free-trial/models/interface';

const AddFreeTrialStudentDialog = ({ user }: { user: ExpandedFreeTrialUsersTableRowData }) => {
  const [openAddressModal, setOpenAddressModal] = React.useState(false);
  const [openDialog, setOpenDialog] = React.useState(false);
  const queryClient = useQueryClient();
  const { patchFreeTrialUser, isPending } = usePatchFreeTrialUser({
    onSuccessCallback: () => {
      toast.success('무료체험학생 수정이 완료되었습니다.');
  ...
  const initialValues = initialEditDialogFormConverter(user);

  const defaultValue: PatchFreeTrialSchema = initialValues;

  const submitForm = (data: PatchFreeTrialSchema) => {
    const partialPatchForm = editFreeTrialUserService.getDirtyFields(initialValues, data);
    const requestDto = patchFreeTrialUserRequestDtoConverter(partialPatchForm);
    patchFreeTrialUser({ freeTrialUserId: user.freeTrialUserDto.id, formData: requestDto });
  };

  const form = useForm({
    defaultValues: defaultValue,
    validators: {
      onMount: patchFreeTrialSchema,
      onChange: patchFreeTrialSchema,
    },
    onSubmit: (data) => {
  ...
  return (
    <Dialog open={openDialog} onOpenChange={setOpenDialog} modal={true}>
      ...
      <DialogContent className="w-full max-h-[70rem]">
        ...
        >
          <DialogHeader>
            <DialogTitle>체험학생 정보 수정하기</DialogTitle>
            <DialogDescription>체험학생의 정보를 수정해주세요.</DialogDescription>
          </DialogHeader>
